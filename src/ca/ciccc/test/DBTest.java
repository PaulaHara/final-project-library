...
/**
 * Create a DB for being used for tests
 *
 * @author paula on 07/05/18.
 */
public class DBTest {
    // JDBC driver name and database URL
    static final String URL = "jdbc:postgresql://localhost/";
    static final String DB_URL = "jdbc:postgresql://localhost/librarytest";
    ...
    private static void createDatabase() throws SQLException {
        ...
        ResultSet rs = stmt.executeQuery("select datname from pg_database where datname like 'librarytest';");

        // If there is no next, than the database doesn't exist and must be created
        if(!rs.next()) {
            stmt.executeUpdate("CREATE DATABASE librarytest");
        }
    }
        ...
        try{
            ...
            if(!tables.next() || !tables.getString("TABLE_NAME").equalsIgnoreCase("customer")) {
                ...
                sql = new String("CREATE TABLE CUSTOMER " +
                        "(id SERIAL, first_name VARCHAR(255), last_name VARCHAR(255), " +
                        "date_of_birth DATE, customer_id VARCHAR(100) UNIQUE, is_active BOOLEAN, PRIMARY KEY (id))");
                stmt.executeUpdate(sql);
            }
            ...
            if(!tables.next() || !tables.getString("TABLE_NAME").equalsIgnoreCase("borrowing")) {
                ...
                sql = new String("CREATE TABLE BORROWING " +
                        ...
                        "return_date DATE NULL, PRIMARY KEY (id), FOREIGN KEY (customer_id) REFERENCES CUSTOMER(id))");
                stmt.executeUpdate(sql);
            }
